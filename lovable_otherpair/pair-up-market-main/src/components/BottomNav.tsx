...
const BottomNav = () => {
  ...
  const items = [
    ...
    { to: "/browse", icon: Search, label: "Listings" },
    { to: "/create", icon: PlusCircle, label: "List", isAction: true },
        ...
        {items.map(({ to, icon: Icon, label, isAction }) => {
          ...
          return (
            ...
            >
              ...
              <span className={`transition-all duration-200 ${isAction ? '-mt-0.5' : ''} ${active ? "font-semibold" : ""}`}>{label}</span>
            </Link>
          );
        })}
      </div>
